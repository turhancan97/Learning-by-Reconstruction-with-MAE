...
def finetune(cfg):
    ...
    wandb_run_name = cfg["logging"]["wandb_run_name"] + '_' + 'finetune' + run_name

    batch_size = cfg["FINETUNE"]["batch_size"]
    load_batch_size = min(cfg["FINETUNE"]["max_device_batch_size"], batch_size)
    ...
    lr_func = lambda epoch: min((epoch + 1) / (cfg["FINETUNE"]["warmup_epoch"] + 1e-8), 0.5 * (math.cos(epoch / cfg["FINETUNE"]["total_epoch"] * math.pi) + 1))
    ...
    for e in range(cfg["FINETUNE"]["total_epoch"]):
        ...
        if avg_val_acc > best_val_acc:
            ...
            torch.save(model, f"{folder_name}/{cfg['FINETUNE']['output_model_path']}")
